...
pub fn map_normals(img: &DynamicImage) -> RgbaImage {
    map_normals_with_strength(img, DEFAULT_STRENGTH)
}

/// Creates the normal mapping from the given image with the given strength.
pub fn map_normals_with_strength(img: &DynamicImage, strength: f32) -> RgbaImage {
    let img = img.clone().into_luma8();
    let mut labpbr_normal_map = RgbaImage::new(img.width(), img.height());

    for (x, y, p) in labpbr_normal_map.enumerate_pixels_mut() {
        let mut new_p = [0.0, 0.0, 0.0, 0.0];
        let s = AdjPixels::new(x, y, &img);

        new_p[0] = s.x_normals();
        new_p[1] = s.y_normals();
        new_p[2] = 0.0; // TODO: AO
        new_p[3] = fetch_pixel(y, x, &img);
        // println!("{:?}", p);
        ...
        p[3] = (new_p[3] * 255.0) as u8;
    }
    labpbr_normal_map
}

fn normalize(v: [f32; 4]) -> [f32; 4] {
    let v_mag = (v[0] * v[0] + v[1] * v[1] + v[2] * v[2] + v[3] * v[3]).sqrt();
    [v[0] / v_mag, v[1] / v_mag, v[2] / v_mag, v[3] / v_mag]
}

fn scale_normalized_to_0_to_1(v: &[f32; 4]) -> [f32; 4] {
    [
        v[0] * 0.5 + 0.5,
        v[1] * 0.5 + 0.5,
        v[2] * 0.5 + 0.5,
